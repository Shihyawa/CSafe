#include "Header/SHA512.h"      //自定义头文件，主要就是包装完的API
...
bool __WhiteList(string VirName, bool USE_NAME) {//读取白名单，有目标程序输出true，没有则输出false
	string ListName;
	string VirusName = get_file_sha512(VirName, USE_NAME);
...
bool WhiteList(string VirName) {//读取白名单，有目标程序输出true，没有则输出false
	...
	if (get_file_sha512(ExeName, false) == get_file_sha512(VirName, false)) {
