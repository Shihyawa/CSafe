		...
		while (true) {
			...
			if (command == "help" || command == "HELP") {
				...
				if (mod == 1) {
					...
					IsConsoleOutOK = false;//禁用日志输出
					std::cout << "Enter 1 to enable high sensitive mode with LSProtect engine, 2 to disable high sensitive mode with LSProtect engine, 3 to set the sensitive value with BITProtect engine, 4 to enable/disable LSProtect: ";
					int code;
					std::cin >> code;
					std::cin.ignore();
					IsConsoleOutOK = true;//启用日志输出
					if (code == 1) {
						...
						IsConsoleOutOK = false;//禁用日志输出
						std::cout << "Please enter the sensitive value with BITProtect engine(float type, default 1.0, more killed with greater, less killed with lesser, set 0.0 to disable the BITProtect engine): ";
